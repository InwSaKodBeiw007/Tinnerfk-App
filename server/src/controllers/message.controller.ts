...
export const MessageController = new Elysia({
    ...
    .ws('/ws', {
        ...
        async message(ws, message) {
            const msg = message as messageModel
            if (!msg.sender) {
                ws.send({ sender: 'system', content: 'Invalid,!!sender!!' })
                return
            }
            if (!msg.recipient) {
                ws.send({ sender: 'system', content: 'Invalid,!!recipient!!' })
                return
            }
            if (!msg.content) {
                ws.send({ sender: 'system', content: 'Invalid,!!content!!' })
                return
            }
            ...
            try {
                ...
                ws.send({ sender: 'system', content: 'Unauthorized,Just a little bit try again na' })
                return
            }
        },
...
const countSubscriber = function (groupName: string): number {
    return groupSubscription.get(groupName)?.size || 0
}

const isRecipientConnected = function (groupName: string, recipient: string): boolean {
    const clients = groupSubscription.get(groupName)
    if (clients)
        return Array.from(clients).find(clients => clients.user_id == recipient) !== undefined
